  ...
  class _ExpensesPageState extends State<ExpensesPage> {
    ...
    List<Map<String, dynamic>> defaultBudgetList =
        []; //fetch items created on database
    ...
    fetchBudgetItemsForToday().then((todayBudgetItems) {
      getDefaultBudget().then((defaultBudgetItems) {
        setState(() {
          budgetItems = [...todayBudgetItems, ...defaultBudgetItems];
        });
      });
    });
      loadItems();
    ...
    Future<List<String>> getDefaultBudget() async {
    final db = await SQLHelper.db();
    final result = await db.query('budget', where: 'id = ?', whereArgs: [1]); // Assuming the auto-generated ID for the initial "Others" category is 1.
    if (result.isNotEmpty) {
      return [result[0]['duty']as String]; // Return the default category as a list.
    }
    return [];
    }
    ...
    Future<List<String>> fetchBudgetItemsForToday() async {
